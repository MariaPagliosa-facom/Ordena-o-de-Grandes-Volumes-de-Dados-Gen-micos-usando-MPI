...
void
distribution_phase(const char* in_path, int size, int rank,
                       long long& N,
                       ...
                       int& LOC_MIN, int& LOC_MAX)
{
    ...
    MPI_Scatterv(
        ...
        MPI_INT,
        local_lens.data(), local_n, MPI_INT,
        0, MPI_COMM_WORLD
    );
    ...
    MPI_Scatterv(
        ...
        MPI_CHAR,
        local_chars.data(), local_bytes, MPI_CHAR,
        0, MPI_COMM_WORLD
    );
    ...
    int pos = 0;
    for (int L : local_lens) {
        local.emplace_back(local_chars.data()+pos, local_chars.data()+pos+L);
        pos += L;
    }
...
void
splitters_phase(const vector<string>& local, int size, int rank,
                    vector<string>& splitters, double& tD)
{
    ...
    MPI_Gather(&samples_count_local, 1, MPI_INT,
            rank==0 ? samples_counts_all.data() : nullptr, 1, MPI_INT,
            0, MPI_COMM_WORLD);
    ...
    MPI_Gatherv(samples_lens.data(), samples_count_local, MPI_INT,
    ...
    MPI_Gatherv(samples_chars.data(), samples_bytes_local, MPI_CHAR,
    ...
    if (rank == 0)
    {
        ...
        for (int r = 0; r < size; ++r)
        {
            for (int i = 0; i < samples_counts_all[r]; ++i)
            {
                ...
                samples_all.emplace_back(samples_chars_all.data() + pos_bytes,
                                        samples_chars_all.data() + pos_bytes + L);
                pos_bytes += L;
            }
        }
        ...
        if (!samples_all.empty() && m > 0) {
            for (int k = 1; k <= m; ++k) {
                ...
                if (idx >= samples_all.size()) idx = samples_all.size() - 1;
    ...
    if (split_count > 0)
    {
        if (rank != 0) { split_lens.resize(split_count); }
        ...
        for (int L : split_lens)
        {
            splitters.emplace_back(split_chars.data() + pos, split_chars.data() + pos + L);
            pos += L;
        }
    }
                       ...
                       int& RECV_MIN, int& RECV_MAX, int& RECV_SUM)
{
    ...
    MPI_Alltoall(sendcounts_len_p.data(),   1, MPI_INT,
                 recvcounts_len_p.data(),   1, MPI_INT, MPI_COMM_WORLD);
    MPI_Alltoall(sendcounts_bytes_p.data(), 1, MPI_INT,
                 recvcounts_bytes_p.data(), 1, MPI_INT, MPI_COMM_WORLD);
    ...
    vector<int>  send_lens_concat  = flatten_2d(buckets_lens,  sdispls_len_p,   sendcounts_len_p);
    vector<char> send_chars_concat = flatten_2d(buckets_chars, sdispls_bytes_p, sendcounts_bytes_p);
    ...
    MPI_Alltoallv(
        send_lens_concat.data(),  sendcounts_len_p.data(),   sdispls_len_p.data(),   MPI_INT,
        recv_lens_concat.data(),  recvcounts_len_p.data(),   rdispls_len_p.data(),   MPI_INT,
        MPI_COMM_WORLD
    );
    MPI_Alltoallv(
        send_chars_concat.data(), sendcounts_bytes_p.data(), sdispls_bytes_p.data(), MPI_CHAR,
        recv_chars_concat.data(), recvcounts_bytes_p.data(), rdispls_bytes_p.data(), MPI_CHAR,
        MPI_COMM_WORLD
    );
...
void
write_phase(const vector<string>& received, const char* out_path,
                         int size, int rank, double& tE, double& tF)
{
    // Empacota meu bloco final
    ...
    MPI_Gather(&out_count_local, 1, MPI_INT,
               rank==0 ? all_counts.data() : nullptr, 1, MPI_INT,
               0, MPI_COMM_WORLD);

    MPI_Gather(&out_bytes_local, 1, MPI_INT,
               rank==0 ? all_bytes.data() : nullptr, 1, MPI_INT,
               0, MPI_COMM_WORLD);
    ...
    MPI_Gatherv(out_lens.data(), out_count_local, MPI_INT,
    ...
    // Gatherv: caracteres
    MPI_Gatherv(out_chars.data(), out_bytes_local, MPI_CHAR,
